...
if (!([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator")) { 
    ...
    Write-Output "A new PowerShell window will open with elevated permissions."
...
if (-Not (Test-Path -Path $steamCmdDir\steamcmd.exe)) {
    ...
    Write-Output "Installing the server (this may take a while)..."
    ...
    Write-Output "SteamCMD already installed, skipping server installation."
    Write-Output "Note: Server updates are no longer performed as CS:GO is no longer updated."
}
...
if ($startMap.Length -eq 0) {
    ...
    if (-Not (Test-Path -Path $startMapDir)) {
        ...
        Invoke-WebRequest -Uri "https://r2.axekz.com/csgo/maps/kz_beginnerblock_go.bsp" -OutFile $startMapDir
    }
}
...
Write-Output ""
Write-Output "=========================================="
Write-Output "GOKZ Server setup completed successfully!"
Write-Output "Server is starting..."
Write-Output "=========================================="
Write-Output ""

# Only pause if running interactively (not from Electron)
if ([Environment]::UserInteractive -and $Host.Name -eq "ConsoleHost") {
    Write-Output "Press any key to close this window..."
    try {
        $null = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")
    } catch {
        # If ReadKey fails, wait a bit instead
        Start-Sleep -Seconds 5
    }
} else {
    # Non-interactive mode - wait a moment to ensure output is captured
    Start-Sleep -Seconds 2
}
